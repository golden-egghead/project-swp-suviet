...
//       // Prepare form data for sending to the API
//       const formData = new FormData();
//       formData.append('data', JSON.stringify({ // Convert the article data to a JSON string and append it to the FormData object
//         title: title,
//         context: context,
//         tagNames: tagNames,
//       }));
...
//       setSuccess('Article submitted for review. Thank you!');
...
//         toast.success('Bài viết đã được gửi cho quản trị viên!');
//         setTitle('');
//         setContext('');
//         setFile(null);
//       } else {
...
const PostArticle = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('context', context);
      formData.append('file', file);
      formData.append('tagNames', tagNames); // Add tagNames to the form data
      ...
      if (response.status === 201) {
        ...
        setContext('');
        setFile(null);
        setTagNames(''); // Clear the tagNames input
      } else {
        setError('Có lỗi xảy ra, vui lòng thực hiện lại!');
      }
    } catch (error) {
